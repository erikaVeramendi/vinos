...
El sistema está orquestado para desplegarse en **Oracle Cloud Infrastructure (OCI)** utilizando Kubernetes (OKE) y automatizado mediante prácticas de **GitOps** con GitHub Actions.
...
* **Base de Datos Relacional:** PostgreSQL (OCI Database) para el modelo transaccional.
* **Base de Datos Analítica:** StarRocks para consultas OLAP/analíticas de alto rendimiento.
* **Almacenamiento (Data Lake):** OCI Object Storage (MinIO compatible) para subida de archivos, documentos e imágenes.
...
* **Orquestador:** Kubernetes, desplegado en OKE (Oracle Kubernetes Engine).
* **CI/CD:** GitHub Actions para integración continua (pruebas, linters, compilación de protobuf) y un flujo completo hacia contenedores y OKE.
...
```text
backend/
├── .github/        # Integración y Despliegue Continuo (CI/CD) con GitHub Actions.
...
├── deploy/         # Infraestructura como Código (Manifiestos de Kubernetes para OKE).
...
│   ├── storage/    #   -> Interfaz HTTP directa para subida masiva de archivos a OCI.
...
└── pkg/            # Utilidades transversales (Conexiones a BD, logs estructurados, interceptores).
```
...
2. **Aislamiento de Responsabilidades (Clean Architecture)**
   ...
   - **Repositorios/Adaptadores (`repository.go`):** Implementan las interfaces esperadas por el dominio. Son los únicos que conocen de PostgreSQL, OCI o llamadas a APIs externas.
